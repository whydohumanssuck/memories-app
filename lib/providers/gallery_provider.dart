...
class GalleryProvider extends ChangeNotifier {
  ...
  Album? get selectedAlbum {
    ...
    return _albums.firstWhere((album) => album.id == _selectedAlbumId, orElse: () => _albums.first);
  }

  List<Photo> get selectedPhotos {
    final album = selectedAlbum;
    return album == null ? [] : photosForAlbum(album.id);
  }

  List<Photo> photosForAlbum(String albumId) {
  ...
  Photo? albumCoverPhoto(Album album) {
    if (album.coverPhotoId.isNotEmpty) {
      final match = _photos.where((photo) => photo.id == album.coverPhotoId);
      if (match.isNotEmpty) return match.first;
    }
    final albumPhotos = photosForAlbum(album.id);
    return albumPhotos.isNotEmpty ? albumPhotos.first : null;
  }
  ...
  void _initializeSampleLibrary() {
    ...
    _photos.addAll([
      ...
      Photo(
        id: 'photo-3',
        title: 'Field light',
        uri: 'https://images.unsplash.com/photo-1496483353456-90997957cf99?auto=format&fit=crop&w=900&q=80',
        albumId: album1.id,
      ),
      Photo(
        id: 'photo-4',
        title: 'Neon street',
        uri: 'https://images.unsplash.com/photo-1496307042754-b4aa456c4a2d?auto=format&fit=crop&w=900&q=80',
        albumId: album2.id,
      ),
      Photo(
        id: 'photo-5',
        title: 'Bridge lights',
        uri: 'https://images.unsplash.com/photo-1470770841072-f978cf4d019e?auto=format&fit=crop&w=900&q=80',
        albumId: album2.id,
      ),
      Photo(
        id: 'photo-6',
        title: 'Metro glow',
        uri: 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=900&q=80',
        albumId: album2.id,
      ),
      Photo(
        id: 'photo-svg',
        title: 'Acid art',
        uri: 'https://dev.w3.org/SVG/tools/svgweb/samples/svg-files/acid.svg',
        albumId: album3.id,
        source: PhotoSource.svg,
      ),
    ]);
  }

  void selectAlbum(String albumId) {
    if (_selectedAlbumId != albumId && _albums.any((album) => album.id == albumId)) {
  ...
  void changeAlbumCover(String albumId, String photoId) {
    final index = _albums.indexWhere((album) => album.id == albumId);
    if (index >= 0 && _photos.any((photo) => photo.id == photoId)) {
  ...
  void restorePhoto(String photoId) {
    final match = _trash.where((photo) => photo.id == photoId);
    if (match.isEmpty) return;
    final restoredPhoto = match.first;
    _trash.removeWhere((photo) => photo.id == photoId);
    _photos.add(restoredPhoto.copyWith(deletedAt: null));
    notifyListeners();
  }
  ...
  void removeAlbum(String albumId) {
    final albumPhotos = photosForAlbum(albumId);
    ...
    if (_selectedAlbumId == albumId) {
      _selectedAlbumId = _albums.isNotEmpty ? _albums.first.id : null;
    }
    notifyListeners();
  }
}
